var RShPortlet = (function ($, Base) {
  ...
  var F = function () {
    ...
    this.$element = $('<div>')
      .data('rsh.portlet', this)
      .attr('id', this.options.id)
      .addClass('rsh-portlet closed')
      .on('click', '.rsh-btn', function () {
        if (that.isReady) {
  ...
  function createTempl() {
    return new RShTemplate([
      '<div class="rsh-portlet-header">',
      '  <#',
      ...
      '    <span>{{ title }}</span>',
      '    <span class="ui-icon ui-icon-pencil rsh-btn" data-action="toggle"></span>',
      '    <span class="ui-icon ui-icon-trash rsh-btn" data-action="remove"></span>',
      '  </h4>',
      '</div>',

      '<div class="rsh-portlet-content">',
      '  <div class="rsh-input-group">',
      '    <label for="{{ data.titleId }}" class="rsh-label">Заголовок</label>',
      '    <input type="text" id="{{ data.titleId }}" class="rsh-input" value="{{ data.title }}">',
      '  </div>',

      '  <div class="rsh-input-group">',
      '    <label for="{{ data.textId }}" class="rsh-label">Текст</label>',
      '    <textarea id="{{ data.textId }}" class="rsh-editor">{{ data.text }}</textarea>',
      '  </div>',

      '  <div class="rsh-input-group">',
      '    <span class="rsh-label">Изображение</span>',
